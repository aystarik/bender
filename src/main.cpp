...
template <uint8_t step_pin, uint8_t dir_pin = IO_NONE> struct Stepper {
    ...
    void setMaxSpeed(float speed) {
        ...
        if (_maxSpeed != speed) {
            ...
            if (_n > 0) {
                _n = (long)(_speed * _speed * _inverse_accel); // Equation 16
                computeNewSpeed();
            }
        }
    }
    ...
    void computeNewSpeed() {
        ...
        if (_n == 0) {
            ...
            _cn -= ((2.0 * _cn) / ((4.0 * _n) + 1)); // Equation 13
            if (_cn < _cmin) {
                _cn = _cmin;
            }
        }
...
Stepper<LED_BUILTIN, IO_B4> s;
Stepper<LED_BUILTIN> s2;
Stepper<LED_BUILTIN> s3;
